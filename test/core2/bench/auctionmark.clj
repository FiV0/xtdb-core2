...
(defn index-item-status-groups [worker]
  (let [{:keys [sut, ^ConcurrentHashMap custom-state]} worker
        ...
        node sut]
    (.putAll custom-state {:item-status-groups (item-status-groups node now)})
...
(defn benchmark [{:keys [seed,
  ...
  (let [duration (Duration/parse duration)
    ...
    {:title "Auction Mark OLTP"
     ...
     [{:t :do
       :stage :load
       :tasks [{:t :call, :f [bcore2/install-tx-fns {:apply-seller-fee tx-fn-apply-seller-fee, :new-bid tx-fn-new-bid}]}
      ...
      (when sync {:t :call, :f #(deref (c2/submit-tx (:sut %) [[:put {:id "auctionmarkd" :finished true}]]))})]}))
