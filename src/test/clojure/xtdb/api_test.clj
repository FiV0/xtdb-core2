...
(t/deftest test-basic-sql-dml
  (letfn [(all-users [tx]
            (->> (xt.sql/q *node* "SELECT u.first_name, u.last_name, u.xt__valid_from, u.xt__valid_to FROM users u"
                           {:basis {:tx tx}})
                 (into #{} (map (juxt :first_name :last_name :xt__valid_from :xt__valid_to)))))]

    (let [tx1 (xt.sql/submit-tx *node* [[:sql "INSERT INTO users (xt__id, first_name, last_name, xt__valid_from) VALUES (?, ?, ?, ?)"
...
(deftest test-sql-insert
  (let [tx1 (xt.sql/submit-tx *node*
                              [[:sql "INSERT INTO users (xt__id, name, xt__valid_from) VALUES (?, ?, ?)"
                                [["dave", "Dave", #inst "2018"]
    ...
    (xt.sql/submit-tx *node*
                      [[:sql "INSERT INTO people (xt__id, renamed_name, xt__valid_from)
                                   SELECT users.id, users.name, users.xt__valid_from
...
(deftest test-sql-insert-app-time-date-398
  (let [tx (xt.sql/submit-tx *node*
                             [[:sql "INSERT INTO foo (xt__id, xt__valid_from) VALUES ('foo', DATE '2018-01-01')"]])]

    (t/is (= (xt/map->TransactionInstant {:tx-id 0, :sys-time (util/->instant #inst "2020-01-01")}) tx))

    (t/is (= [{:xt__id "foo", :xt__valid_from (util/->zdt #inst "2018"), :xt__valid_to (util/->zdt util/end-of-time)}]
             (xt.sql/q *node* "SELECT foo.xt__id, foo.xt__valid_from, foo.xt__valid_to FROM foo")))))
...
(deftest test-dml-default-all-app-time-flag-339
  (let [tt1 (util/->zdt #inst "2020-01-01")
    ...
    (letfn [(q []
              (set (xt.sql/q *node*
                             "SELECT foo.version, foo.xt__valid_from, foo.xt__valid_to FROM foo")))]
      (xt.sql/submit-tx *node*
                        [[:sql "INSERT INTO foo (xt__id, version) VALUES (?, ?)"
                          [["foo", 0]]]])

      (t/is (= #{{:version 0, :xt__valid_from tt1, :xt__valid_to eot}}
               (q)))
      ...
      (t/testing "update as-of-now"
        ...
        (t/is (= #{{:version 0, :xt__valid_from tt1, :xt__valid_to tt2}
                   {:version 1, :xt__valid_from tt2, :xt__valid_to eot}}
                 (q))))
      ...
      (t/testing "`FOR PORTION OF` means flag is ignored"
        (xt.sql/submit-tx *node*
                          [[:sql (str "UPDATE foo "
                            ...
                            [[tt1 tt2]]]]
                          {:default-all-app-time? false})
        (t/is (= #{{:version 2, :xt__valid_from tt1, :xt__valid_to tt2}
                   {:version 1, :xt__valid_from tt2, :xt__valid_to eot}}
                 (q))))
      ...
      (t/testing "UPDATE for-all-time"
        ...
        (t/is (= #{{:version 3, :xt__valid_from tt1, :xt__valid_to tt2}
                   {:version 3, :xt__valid_from tt2, :xt__valid_to eot}}
                 (q))))
      ...
      (t/testing "DELETE as-of-now"
        ...
        (t/is (= #{{:version 3, :xt__valid_from tt1, :xt__valid_to tt2}
                   {:version 3, :xt__valid_from tt2, :xt__valid_to tt5}}
                 (q))))
      ...
      (t/testing "UPDATE FOR ALL APPLICATION_TIME"
        ...
        (t/is (= #{{:version 4, :xt__valid_from tt1, :xt__valid_to tt2}
                   {:version 4, :xt__valid_from tt2, :xt__valid_to tt5}}
                 (q))))
...
(deftest test-dql-as-of-now-flag-339
  (let [tt1 (util/->zdt #inst "2020-01-01")
    ...
    (t/is (= [{:version 1, :xt__valid_from tt2, :xt__valid_to eot}]
             (xt.sql/q *node*
                       "SELECT foo.version, foo.xt__valid_from, foo.xt__valid_to FROM foo"
                       {:default-all-app-time? false})))
    ...
    (t/is (= [{:version 0, :xt__valid_from tt1, :xt__valid_to tt2}]
             (xt.sql/q *node*
                       (str "SELECT foo.version, foo.xt__valid_from, foo.xt__valid_to "
    ...
    (t/is (= [{:version 0, :xt__valid_from tt1, :xt__valid_to tt2}
              {:version 1, :xt__valid_from tt2, :xt__valid_to eot}]
             (xt.sql/q *node*
                       "SELECT foo.version, foo.xt__valid_from, foo.xt__valid_to
...
(t/deftest test-erase
  (letfn [(q [tx]
            (set (xt.sql/q *node*
                           "SELECT foo.xt__id, foo.version, foo.xt__valid_from, foo.xt__valid_to FROM foo"
                           {:basis {:tx tx}})))]
    (let [tx1 (xt.sql/submit-tx *node*
                                [[:sql "INSERT INTO foo (xt__id, version) VALUES (?, ?)"
                                  [["foo", 0]
          ...
          tx2 (xt.sql/submit-tx *node*
                                ...
                                {:default-all-app-time? false})
          v0 {:version 0,
              :xt__valid_from (util/->zdt #inst "2020-01-01"),
              :xt__valid_to (util/->zdt #inst "2020-01-02")}

          v1 {:version 1,
              :xt__valid_from (util/->zdt #inst "2020-01-02"),
...
(t/deftest aborts-insert-if-end-lt-start-401-425
  (letfn [(q-all []
            (->> (xt.sql/q tu/*node* "SELECT foo.xt__id, foo.xt__valid_from, foo.xt__valid_to FROM foo")
                 (into {} (map (juxt :xt__id (juxt :xt__valid_from :xt__valid_to))))))]

    (xt.sql/submit-tx tu/*node* [[:sql "INSERT INTO foo (xt__id) VALUES (1)"]])

    (xt.sql/submit-tx tu/*node* [[:sql "
INSERT INTO foo (xt__id, xt__valid_from, xt__valid_to)
