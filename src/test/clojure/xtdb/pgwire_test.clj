...
(deftest set-app-time-defaults-test
  (with-open [conn (jdbc-conn)]
    (let [sql #(q conn [%])]
      ...
      (is (= [{:version 1, :xt__valid_from "2020-01-02T00:00Z", :xt__valid_to "9999-12-31T23:59:59.999999Z"}]
             (q conn ["SELECT foo.version, foo.xt__valid_from, foo.xt__valid_to FROM foo"])))

      (sql "SET application_time_defaults iso_standard")
      (is (= (set [{:version 0, :xt__valid_from "2020-01-01T00:00Z", :xt__valid_to "2020-01-02T00:00Z"}
                   {:version 1, :xt__valid_from "2020-01-02T00:00Z", :xt__valid_to "9999-12-31T23:59:59.999999Z"}])
             (set (q conn ["SELECT foo.version, foo.xt__valid_from, foo.xt__valid_to FROM foo"])))))))
