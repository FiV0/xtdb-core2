...
Inserted data can be forced to live in the past or future by setting `xt__valid_from` and/or `xt__valid_to`:
...
////
Updates can be forced to occur in the past or future by setting `XT__VALID_FROM` and/or `XT__VALID_TO`:
[source,sh]
----
UPDATE users SET name='James', xt__valid_from=DATE '1998-01-05' WHERE users.id=1234;
----
////
